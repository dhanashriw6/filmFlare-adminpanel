...
import { getCommission, updateCommission } from "@/api/commission";
...
const EditCommission = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const all = comRes.data?.data || comRes.data || [];
        ...
        setForm({
          ...
          type: target.type ?? "fixed",
          amount_per_hour: target.amount_per_hour ?? "",
          amount_per_half_day: target.amount_per_half_day ?? "",
          amount_per_day: target.amount_per_day ?? "",
          cap_per_hour: target.cap_per_hour ?? "",
          cap_per_half_day: target.cap_per_half_day ?? "",
          cap_per_day: target.cap_per_day ?? "",
        });
      } catch {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      await updateCommission(Number(id), payload);
      setSuccess(true);
      setTimeout(() => router.push("/masters/commission"), 1200);
    } catch (err) {
      setError(err?.response?.data?.message || "Failed to update commission. Please try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <DashboardLayout>
      <div className="cc-page">
            ...
            <div className="cc-card-header">
              <div className="cc-icon-wrap">
                <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
                  <path d="M4 13.5V17h3.5l7-7-3.5-3.5-7 7zM16.5 6.5a1 1 0 000-1.414l-2.086-2.086a1 1 0 00-1.414 0L11.5 4.5 15 8l1.5-1.5z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
              <div>
                <h2 className="cc-title">Edit Commission</h2>
                <p className="cc-subtitle">Update commission rates for this category</p>
              </div>
            </div>

            {/* Fetching state */}
            {fetching ? (
              <div className="cc-fetch-state">
                <div className="cc-spinner cc-spinner--lg" />
                <span>Loading commission…</span>
              </div>
            ) : (
              <>
                {/* Success */}
                {success && (
                  <div className="cc-alert cc-alert--success">
                    ...
                    Commission updated! Redirecting…
                  </div>
                )}
        ...
        <style>{`
          ...
          .cc-input--prefixed { padding-left: 1.75rem; }
          .cc-toggle-group {
            display: flex;
            border: 1.5px solid #e5e7eb;
            border-radius: 10px;
          ...
          .cc-toggle + .cc-toggle { border-left: 1.5px solid #e5e7eb; }
          .cc-toggle--active {
