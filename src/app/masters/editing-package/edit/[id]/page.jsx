...
import {
    ...
    EditingPackageFeature,
} from "@/api/editingPackage";
...
const EditEditingPackage = () => {
    ...
    // Unified image list: existing server images + new uploads
    const [images, setImages] = useState([]);
    // Track which existing image keys to delete on save
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                setPageLoading(true);
                const res = await getEditingPackageDetails(packageId);
                const pkg = res.data?.data?.editing_package;
                setForm({
                    name: pkg?.name || "",
                ...
                setPageError("Failed to load package details. Please try again.");
            } finally {
    ...
    const addFeature = () =>
        setForm((f) => ({
            ...
            features: [
                ...f.features,
                { feature_key: "edited_photos", quantity: 1, label: "", sort_order: f.features.length },
            ],
        }));
    ...
    const removeFeature = (i) =>
        setForm((f) => ({
            ...
            features: f.features
                .filter((_, idx) => idx !== i)
                .map((feat, idx) => ({ ...feat, sort_order: idx })),
        }));
    ...
    const handleFilePick = (e) => {
        ...
        newEntries.forEach((entry) => {
            if (entry.type === "upload") uploadImage(entry.id, entry.file);
        });
    };

    const uploadImage = async (slotId, file) => {
        try {
    ...
    const retryUpload = (entry) => {
        setImages((prev) =>
            prev.map((img) =>
                img.type === "upload" && img.id === entry.id
                    ? { ...img, status: "uploading" }
                    : img
            )
        );
        uploadImage(entry.id, entry.file);
    };
    ...
    const removeImage = (entry) => {
        if (entry.type === "existing") {
            ...
            setImages((prev) =>
                prev.filter((img) => img.id !== entry.id)
            );
        }
    };
    ...
    const validate = () => {
        ...
        if (uploads.some((s) => s.status === "uploading")) return "Please wait for images to finish uploading.";
        if (uploads.some((s) => s.status === "error")) return "Some images failed to upload. Retry or remove them.";
        return null;
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const deletedImages = removedImageIds.map((id) => ({
                id,
            }));
            ...
            await updateEditingPackage(packageId, {
                ...form,
                ...
                <div className="cp-page">
                    <div className="cp-card">
                        ...
                        <div className="cp-state cp-state--error">
                            ...
                            <span>{pageError}</span>
                            <button className="cp-btn cp-btn--ghost" onClick={() => router.back()}>Go back</button>
                        </div>
                    </div>
                </div>
            </DashboardLayout>
    ...
    return (
        <DashboardLayout>
            <div className="cp-page">
                ...
                <div className="cp-card">
                    ...
                    <div className="cp-card-body">
                        ...
                        <div className="cp-card-header">
                            <div className="cp-icon-wrap">
                                <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
                                    <path d="M4 17.5L7.5 17l9-9a2.121 2.121 0 00-3-3l-9 9L4 17.5z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
                                    <path d="M13.5 5.5l3 3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                                </svg>
                            </div>
                            <div>
                                <h2 className="cp-title">Edit Editing Package</h2>
                                <p className="cp-subtitle">Update package details, features, and images</p>
                            </div>
                        </div>
                        ...
                        {/* Show existing + new images in one grid */}
                        {images.length > 0 && (
                            <div className="cp-img-grid">
                                {images.map((entry) => {
                                    if (entry.type === "existing") {
                                        return (
                                            <div key={entry.key} className="cp-img-tile cp-img-tile--done">
                                                <img src={entry.url} alt="" className="cp-img-thumb" />
                                                ...
                                                <div className="cp-img-actions">
                                                    <button
                                                        type="button"
                                                        className="cp-img-btn cp-img-btn--remove"
                                    ...
                                    // upload slot
                                    return (
                                        <div key={entry.id} className={`cp-img-tile cp-img-tile--${entry.status}`}>
                                            <img src={entry.preview} alt="" className="cp-img-thumb" />
                                            ...
                                            <div className="cp-img-actions">
                                                {entry.status === "error" && (
                                                    <button type="button" className="cp-img-btn cp-img-btn--retry" onClick={() => retryUpload(entry)} title="Retry">
                                                        <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                                                ...
                                                <button type="button" className="cp-img-btn cp-img-btn--remove" onClick={() => removeImage(entry)} title="Remove">
                                                    <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                                                        <path d="M2 2l8 8M10 2l-8 8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                                                    </svg>
                                                </button>
                        ...
                        >
                            ...
                            <p className="cp-dropzone-title">Add more images</p>
          ...
          .cp-state { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.75rem; padding: 4rem 1rem; color: var(--color-gray); font-size: 0.875rem; }
          .cp-state--error { color: #dc2626; }
          ...
          /* Drop zone */
          .cp-dropzone { border: 1.5px dashed #d1d5db; border-radius: 12px; background: var(--color-ivory, #fffef5); padding: 1.25rem 1rem; display: flex; flex-direction: column; align-items: center; gap: 0.35rem; cursor: pointer; transition: border-color 0.18s, background 0.18s; }
...
export default EditEditingPackage;